...
# This can be solved using inorder traversal, but its complexity will be O(n).
# (kth largest element in bst)
# Used : Assume that the root is having N nodes in its left subtree.
#        If K = N + 1, root is K-th node.
#        If K < N, we will continue our search (recursion) for the Kth smallest element in
#        the left subtree of root.
#        If K > N + 1, we continue our search in the right subtree for the (K - N - 1)-th
#        smallest element.
