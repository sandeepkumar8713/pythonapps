...
# Question : The problem is to find maximum sum sub-array divisible by k and then return (sum / k). Given n boxes
# containing some chocolates arranged in a row. There are k number of students. The problem
# is to distribute maximum number of chocolates equally among k students by selecting a consecutive sequence of
# boxes from the given lot. Consider the boxes are arranged in a row with numbers from 1 to n from left to right.
# We have to select a group of boxes which are in consecutive order that could provide maximum number of chocolates
# equally to all the k students. An array arr[] is given representing the row arrangement of the boxes and arr[i]
