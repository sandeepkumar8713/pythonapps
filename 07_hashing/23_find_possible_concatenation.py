...
# Question : You are given a string S, and a list of words L i.e array/vector of strings
...
#        2. Traverse through all possible substrings in string S which are equal to
#            size_L(total number of characters produced if all the words in list L are
#            concatenated).
...
#           we decrease it's corresponding count, if it's not present in temp_hash_map we
#           simply break.
#        5. After traversing the substring we traverse temp_hash_map and look for any key
#           which has it's count > 0. If we found no such key it means that all the words
#           in list L were found in substring and store the given starting index of
#           the substring, if we find a key which has it's count > 0 it means we did
#           not traversed whole substring because we came across a word which was not in
#           temp_hash_map.
# Complexity : O(N - K) * K
