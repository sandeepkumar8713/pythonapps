...
# such that 0 <= P < Q < N. A slice (P, Q) of array A is called arithmetic if the sequence:
...
#           if inpArr[i] - inpArr[i-1] == inpArr[i-1] - inpArr[i-2]:
#               runningCount += 1
#               total += runningCount
#           else: runningCount = 0
#        return total
...
def numberOfArithmeticSlices(inpArr):
    runningCount = 0
    total = 0
    for i in range(2, len(inpArr)):
        if inpArr[i] - inpArr[i-1] == inpArr[i-1] - inpArr[i-2]:
