# Question : Given a snake and ladder board, find the minimum number of dice throws required
# to reach the destination or last cell from source or 1st cell. Basically, the player
# has total control over outcome of dice throw and wants to find out minimum number of
# throws required to reach last cell.
#
# Question Type : Easy
...
#        Push the starting cell in queue and start a loop to do BFS over it.
#        Loop while queue is empty or target is reached. In loop pop a cell from queue. Now
#           loop over all the possible moves it can make from that cell. i.e. from 1 to 6.
#           If the next pos is not yet visited, make a new cell(object) for it, taking into
#           consideration whether the next pos goes to snake or ladder or stays there.
