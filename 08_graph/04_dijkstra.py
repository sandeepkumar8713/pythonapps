...
#           (minimum value in minDist array which is not yet added in SPT) and include it in SPT. Now use this newly
#           found vertex, to check if via (lesser cost) path is possible, by looping over all the vertices which are not
#           yet included in SPT and update minDist.
# Complexity : O(n^2)
#              If the input graph is represented using adjacency list, it can be reduced to O(E log V) with the help of
#              binary heap.
