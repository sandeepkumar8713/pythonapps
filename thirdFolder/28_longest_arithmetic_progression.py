...
# Question : Given an array A of integers, return the length of the longest arithmetic subsequence in A.
...
#        difference d already.
#        If yes, we can then extend the existing chain length by 1.
#        Else, if not, then we can start a new chain of length 2 with this new difference d and (A[j], A[i]) as its
#        elements.
