...
# Used : L is chain length. Here we are taking variable length and start at different i.
#        Place parenthesis at different places between first and last matrix(i,j), recursively
#        calculate count of multiplications for each parenthesis placement and return the minimum count.
#        bracket[i][j] tells where to keep the bracket.
...
#        printParenthesis(i, j, bracket, matrixNameAscii):
#        if i == j : print name, increment name, return
#        print "(",
    ...
    for L in range(2, n):
        for i in range(1, n - L + 1):
            ...
            for k in range(i, j):
                ...
                # already multiplied
                q = dp[i][k] + dp[k + 1][j] + matrixSize[i - 1] * matrixSize[k] * matrixSize[
                    j]  # matrix multiplication cost
                if q < dp[i][j]:
                    dp[i][j] = q
                    bracket[i][j] = k
