...
# CTCI : Q17_13_ReSpace
# Question : Oh, no! You have accidentally removed all spaces, punctuation, and capitalization in a
# lengthy document. A sentence like "I reset the computer. It still didn't boot!"
# became "iresetthecomputeritstilldidntboot". You'll deal with the punctuation and capitalization later;
...
#        	c = sentence[index]
#         	partial += c, invalid = 0
#           if partial not in setOfWords:
#           	invalid = len(partial)
#         	if invalid < bestInvalid:
#           	parseResult = split(setOfWords, sentence, index + 1, memo)
...
def split(setOfWords, sentence, start, memo):
...
def bestSplit(setOfWords, sentence):
    memo = [None] * len(sentence)
    parseResult = split(setOfWords, sentence, 0, memo)
