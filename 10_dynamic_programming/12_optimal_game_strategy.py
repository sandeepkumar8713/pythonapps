...
#               End pointers here are i and j. User can choose either i or j. If he choose i, opponent while be left
#               with i+1 to j to choose from. So opponent can choose either i+1 or j. So user will be left with x or y.
#
...
#               So the current value should be: dp[i][j] = max(arr[i] + min(x, y), arr[j] + min(y, z))
#        return dp[0][n-1]
# Complexity : O(n * n)
